...
def best_path(maze, model):
    ...
    while True:
        ...
        best_child_and_value = max([ (value, cid) for cid,value in child_and_value ]) #Since value is first in tuple, it will be prioritized.
        best_child = best_child_and_value[1] #Remove value, only want the node
...
def visualize_juxtaposed_best_paths(maze, models= eu_du_pwu):
    ...
    for ax, model in zip(axs, models):
        path = best_path(maze, model) # Get the best path
